...
```{r, include = FALSE}
missing <- readRDS("rds/missing.rds")
soil.int <- readRDS("rds/data.rds")
error_rate_k <- readRDS("rds/k_plot.rds")
lda <- readRDS("rds/lda.rds")
training <- readRDS("rds/ldatr.rds")
tmp <- readRDS("rds/tmp.rds")
tree_model <- readRDS("rds/tree_model.rds")
randomforest_model <- readRDS("rds/randomforest_model.rds")
soilcat <- readRDS("rds/soilcat.rds")
soilcon <- readRDS("rds/soilcon.rds")
soilcon2 <- readRDS("rds/soilcon2.rds")
model1 <- readRDS("rds/model1.rds")
model2 <- readRDS("rds/model2.rds")
model3 <- readRDS("rds/model3.rds")
model4 <- readRDS("rds/model4.rds")
soil_test <- readRDS("rds/soil_test.rds")
```
...
* **變數說明：**原始資料扣除ID、名稱、重複及有明顯共線性(soil erosion = watereros + winderos)之變數以後，具有15個類別變數、16個連續變數，共有31個實質變數，以下為各變數的說明。
